...
public class ProfileService {
    ...
    public String getUserEvaluation(String userEmail) {
        String prompt = generatePrompt(userEmail);
        return getRecommendationFromOpenAi(prompt);
    }

    private String generatePrompt(String userEmail) {
        LocalDate endDate = LocalDate.now();
        LocalDate startDate = endDate.minusDays(7);

        // 사용자의 메모(1주일 기록) 가져오기
        List<MemoEntity> memoList = memoRepository.findByUserEmailAndDateBetween(userEmail, startDate.toString(), endDate.toString());
